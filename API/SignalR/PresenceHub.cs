...
public class PresenceHub(PresenceTracker tracker) : Hub
{
    ...
    public override async Task OnDisconnectedAsync(Exception? exception)
    {
        ...
        var isOnline = await tracker.UserDisconnected(Context.User.GetUsername(), Context.ConnectionId);
        if (isOnline) await Clients.Others.SendAsync("UserIsOffline", Context.User?.GetUsername());
